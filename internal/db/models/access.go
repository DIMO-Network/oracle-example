...
var AccessWhere = struct {
	...
	Wallet: whereHelperstring{field: "\"oracle_example\".\"access\".\"wallet\""},
}
...
var (
	accessAllColumns            = []string{"wallet"}
	accessColumnsWithoutDefault = []string{"wallet"}
	accessColumnsWithDefault    = []string{}
	accessPrimaryKeyColumns     = []string{"wallet"}
...
func Accesses(mods ...qm.QueryMod) accessQuery {
	mods = append(mods, qm.From("\"oracle_example\".\"access\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"oracle_example\".\"access\".*"})
	}
...
func FindAccess(ctx context.Context, exec boil.ContextExecutor, wallet string, selectCols ...string) (*Access, error) {
	accessObj := &Access{}
	...
	query := fmt.Sprintf(
		"select %s from \"oracle_example\".\"access\" where \"wallet\"=$1", sel,
	)
...
func (o *Access) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"oracle_example\".\"access\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"oracle_example\".\"access\" %sDEFAULT VALUES%s"
		}
...
func (o *Access) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"oracle_example\".\"access\" SET %s WHERE %s",
...
func (o AccessSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"oracle_example\".\"access\" SET %s WHERE %s",
...
func (o *Access) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(accessPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert access, could not build conflict column list")
			}

			conflict = make([]string, len(accessPrimaryKeyColumns))
			copy(conflict, accessPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"oracle_example\".\"access\"", updateOnConflict, ret, update, conflict, insert, opts...)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert access")
	}
...
func (o *Access) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"oracle_example\".\"access\" WHERE \"wallet\"=$1"
...
func (o AccessSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"oracle_example\".\"access\" WHERE " +
...
func (o *Access) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindAccess(ctx, exec, o.Wallet)
	if err != nil {
		return err
	}
...
func (o *AccessSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"oracle_example\".\"access\".* FROM \"oracle_example\".\"access\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, accessPrimaryKeyColumns, len(*o))
...
func AccessExists(ctx context.Context, exec boil.ContextExecutor, wallet string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"oracle_example\".\"access\" where \"wallet\"=$1 limit 1)"
...
func (o *Access) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return AccessExists(ctx, exec, o.Wallet)
}
